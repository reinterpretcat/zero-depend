...
impl<T> Tensor<T> {
    ...
    pub fn arange(end: T) -> Result<Tensor<T>>
    where
    ...
    pub(crate) fn compute_strides(shape: &[usize]) -> SmallVec<usize, 4> {
...
impl<T, const N: usize> TryFrom<Vec<[T; N]>> for Tensor<T> {
    type Error = TensorError;

    fn try_from(data: Vec<[T; N]>) -> Result<Self> {
        if data.is_empty() {
...
mod tests {
    ...
    fn test_arange_f32() -> Result<()> {
        let tensor = Tensor::arange(5_f32)?;
        assert_eq!(tensor.shape(), &[5]);
        assert_eq!(*tensor.get(&[0])?, 0.);
        assert_eq!(*tensor.get(&[4])?, 4.);
